...
export default function Page() {
  const [emails, setEmails] = useState<CategorizedInquiry[]>([]);
  ...
  const fetchEmails = useCallback(async (force: boolean) => {
    setLoading(true);
    setError(null);
    try {
      ...
      toast.error('Failed to fetch emails', { description: err.message });
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  const testConnection = useCallback(async () => {
    ...
    try {
      ...
      toast.error('IMAP connection failed', { description: err.message });
    } finally {
      setTesting(false);
    }
  }, []);
  ...
  }, [emails, search, categoryFilter, priorityFilter]);
  ...
  // ── Render ──
  return (
    <div className="min-h-screen flex flex-col bg-gradient-to-br from-emerald-50/40 via-white to-teal-50/30 dark:from-zinc-950 dark:via-zinc-900 dark:to-zinc-950">
      <Toaster richColors position="top-right" />

      {/* Header */}
      <header className="sticky top-0 z-30 border-b border-emerald-100/60 dark:border-zinc-800 bg-white/80 dark:bg-zinc-950/80 backdrop-blur-xl">
        <div className="max-w-[1400px] mx-auto px-4 sm:px-6 py-3 flex items-center justify-between gap-4 flex-wrap">
          <div className="flex items-center gap-3">
            <div className="size-10 rounded-xl bg-gradient-to-br from-emerald-500 to-teal-600 flex items-center justify-center shadow-lg shadow-emerald-500/20">
              <Mailbox className="size-5 text-white" />
            </div>
            <div>
              <h1 className="text-lg sm:text-xl font-bold tracking-tight text-zinc-900 dark:text-white">
                Email Inquiry Dashboard
              </h1>
              <p className="text-xs text-zinc-500 dark:text-zinc-400 flex items-center gap-1.5 flex-wrap">
                <Building2 className="size-3" />
                <span className="font-medium text-emerald-700 dark:text-emerald-400">ecomruns.com</span>
                <span className="text-zinc-400">↔</span>
                <span className="font-medium text-teal-700 dark:text-teal-400">techichamps.com</span>
                <span className="text-zinc-400">·</span>
                <span className="text-zinc-500">info@ecomruns.com</span>
              </p>
            </div>
          </div>
      ...
      <main className="flex-1 max-w-[1400px] mx-auto w-full px-4 sm:px-6 py-6">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3 sm:gap-4 mb-6">
          <StatCard
            ...
            tone="emerald"
            sub={fetchedAt ? `Updated ${formatRelative(fetchedAt)}` : '—'}
          />
          <StatCard
            label="Urgent / High"
            value={priorities.urgent + priorities.high}
            icon={<AlertCircle className="size-5" />}
            tone="red"
            sub={`${priorities.urgent} urgent · ${priorities.high} high`}
          />
          <StatCard
            label="Categories"
            ...
            tone="purple"
            sub={categories[0] ? `Top: ${categories[0][0]}` : '—'}
          />
          <StatCard
            label="Cache Status"
            value={cached ? 'Cached' : 'Fresh'}
            icon={<Sparkles className="size-5" />}
            tone="teal"
            sub={cached ? '2-min TTL active' : 'Live from IMAP'}
          />
        </div>

        {/* Error Banner */}
        {error && (
          <div className="mb-6 rounded-xl border border-red-200 bg-red-50 dark:bg-red-950/40 dark:border-red-900 p-4 flex items-start gap-3">
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-xs text-red-600 dark:text-red-500 mt-2">
                Check IMAP credentials in <code className="px-1 py-0.5 bg-red-100 dark:bg-red-900/60 rounded">.env</code> and that
                <code className="px-1 py-0.5 bg-red-100 dark:bg-red-900/60 rounded mx-1">imap.hostinger.com:993</code>
        ...
        {testResult && (
          <div
            className={cn(
              'mb-6 rounded-xl border p-3 flex items-start gap-3 text-sm',
              testResult.ok
                  ...
                  <div className="divide-y divide-zinc-100 dark:divide-zinc-800">
                    {filtered.map((e) => (
                      <button
                        ...
                        className={cn(
                          'w-full text-left p-4 hover:bg-emerald-50/50 dark:hover:bg-zinc-800/50 transition-colors flex gap-3 group',
                          selectedId === e.id && 'bg-emerald-50 dark:bg-zinc-800/70 border-l-2 border-emerald-600'
                        )}
                      >
      ...
      {/* Footer */}
      <footer className="mt-auto border-t border-emerald-100/60 dark:border-zinc-800 bg-white/60 dark:bg-zinc-950/60 py-4">
        <div className="max-w-[1400px] mx-auto px-4 sm:px-6 text-xs text-zinc-500 dark:text-zinc-400 flex items-center justify-between flex-wrap gap-2">
          <span>
            IMAP: <code className="px-1 py-0.5 bg-zinc-100 dark:bg-zinc-800 rounded">imap.hostinger.com:993</code> ·
            Filtering: <code className="px-1 py-0.5 bg-zinc-100 dark:bg-zinc-800 rounded">@techichamps.com</code>
          </span>
          {fetchedAt && (
            <span>Last sync: {formatDate(fetchedAt)}{cached ? ' (cached)' : ''}</span>
          )}
        </div>
      </footer>
    </div>
...
function StatCard({
  ...
  tone: 'emerald' | 'red' | 'purple' | 'teal';
  sub?: string;
}) {
  const toneMap = {
    emerald: 'from-emerald-500 to-emerald-600 text-white shadow-emerald-500/30',
    red: 'from-red-500 to-rose-600 text-white shadow-red-500/30',
    purple: 'from-purple-500 to-fuchsia-600 text-white shadow-purple-500/30',
    teal: 'from-teal-500 to-cyan-600 text-white shadow-teal-500/30',
  };
  return (
    <Card className="border-emerald-100/80 dark:border-zinc-800 overflow-hidden">
      <CardContent className="p-4 flex items-center gap-3">
        <div className={cn('size-10 rounded-xl bg-gradient-to-br flex items-center justify-center shadow-md', toneMap[tone])}>
          {icon}
        </div>
        <div className="min-w-0 flex-1">
          <p className="text-xs text-zinc-500 dark:text-zinc-400 truncate">{label}</p>
          <p className="text-xl font-bold text-zinc-900 dark:text-white leading-tight truncate">{value}</p>
